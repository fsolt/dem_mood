...
cri <- claassen_replication_input$data %>% 
...
ggplot(no_problems, aes(x = prop_ours, y = prop_cls, color = p_dcpo)) + 
    geom_point(shape = 19,
               alpha = .2) + 
    theme_bw() +
    labs(y = "Proportion Supporting Democracy in Claassen's Replication File",
         x = "Proportion Supporting Democracy in Currently Available Survey Data") +
    annotate("text", x = .22, y = .95, label = "evdemoc_asiab accounts\nfor most overreports") +
    annotate("text", x = .75, y = .23, label = "impdemoc_pew accounts for\nmost large underreports") +
    scale_color_discrete(name="Survey\nProgram")

ggsave('paper/ourdata_vs_claassensdata.pdf', width = 7)

needed <- anti_join(sd, cri, by = c("country", "year", "item_fam", "project")) # 311 obs 
...
available <- anti_join(cri, sd, by = c("country", "year", "item_fam", "project")) #1420 obs
...
compare_supdem <- read.csv("~/Sources/FSLibrary/Claassen2020/dem_mood_apsr.csv") %>%
