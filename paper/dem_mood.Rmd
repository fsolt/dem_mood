...
title: |
  | Degrees of Democratic Support vs Degrees of Researcher Freedom:
  | A Sensitivity Analysis of Thermostatic Public Opinion in 
  | Response to Democratic Change
...
base_input_raw <- claassen_input_raw %>%
...
miss <- c("lower", "upper", "mar", "theory") %>% # missing-data treatments
    set_names()
...
```{r orig_cm5, eval=FALSE, cache=FALSE, include=FALSE, results=FALSE}
...
ttt <- unlist(m1_1_tidy) %>%
  tibble(temp = names(.), values = ., names = names(.)) %>%
  filter(!str_detect(temp, "_z")) %>% 
  separate_wider_regex(temp, c(by = "\\w+", ".",
                               miss = "\\w+", ".",
                               var = "\\D*",
                               term = "\\d")) %>% 
  filter(!str_detect(var, "term|conf"))


  
map(m1_1_list, function(aResult) {
  tidy_result <- tidy(aResult, conf.int = TRUE) %>% 
    mutate(std.error = vcovHC_se(aResult))
