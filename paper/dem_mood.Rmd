...
The darkest, left-most coefficients in the left-most cluster represent the combination of mixed coding rule and treatment of non-responses as unsupportive of democracy, that is, the combination employed in @Claassen2020b.
...
\newpage
\blandscape
```{r df_apsr_list12, eval=FALSE}
df_apsr_list12 <- map(by,
                      \(b) {
                        map(miss,
                            \(m) {
                              df_apsr %>%
...
df_apsr_list12[[1]][[1]] <- df_apsr %>% 
  select(Country, Year, SupDem_trim, Libdem_z, lnGDP_imp, Corrup_TI_z) %>% 
  filter(., complete.cases(.))

rio::export(df_apsr_list12, file = here("data",
                                        "df_apsr_list12.rds"))

```

```{r regression_results_m12}
df_apsr_list12 <- rio::import(here("data", "df_apsr_list12.rds"))
                   ...
                   map(miss,
                       \(m) {
                         plm(diff(SupDem_trim, lag = 1) ~ 
                               ...
                               diff(lnGDP_imp, lag = 1) +
                               plm::lag(lnGDP_imp, 1) +
                               diff(Corrup_TI_z, lag = 1) +
                               plm::lag(Corrup_TI_z, 1),
                             model = 'pooling',
                             data = pdata.frame(df_apsr_list12[[b]][[m]],
                                                index = c("Country", "Year")))
                       })
                 })
...
m1_2_tidy <- map(by,
                 \(b) {
                   map(miss,
                       \(m) {
                         tidy_result <- tidy(m1_2_list[[b]][[m]],
                                             conf.int = TRUE) %>% 
                           mutate(std.error = vcovHC_se(m1_2_list[[b]][[m]]),
                                  model = paste(b, m),
                                  coding = b,
                                  treatment = m) %>% 
                           filter(str_detect(term, "Libdem"))
...
```{r df_apsr_list13, eval=FALSE}
df_apsr_list13 <- map(by,
                      \(b) {
                        map(miss,
                            \(m) {
                              df_apsr %>%
...
df_apsr_list13[[1]][[1]] <- df_apsr %>% 
  select(Country, Year, SupDem_trim, Libdem_z, lnGDP_imp) %>% 
  filter(., complete.cases(.))

rio::export(df_apsr_list13, file = here("data",
                                        "df_apsr_list12.rds"))

```


```{r regression_results_m13}
df_apsr_list13 <- rio::import(here("data", "df_apsr_list13.rds"))
...
m1_3_tidy <- map(by,
                 \(b) {
                   map(miss,
                       \(m) {
                         tidy_result <- tidy(m1_3_list[[b]][[m]],
                                             conf.int = TRUE) %>% 
                           mutate(model = paste(b, m),
                                  coding = b,
                                  treatment = m)
                       }) %>% 
                     bind_rows()
