...
Data wrangling, the task of getting the data needed into the format required to perform analyses.
...
With democracy under increasing threat in countries around the world, how publics react is a crucial question.
...
In contrast, one prominent recent study, @Claassen2020b, argues instead that democratic support behaves thermostatically, that is, that increases in democracy yield an authoritarian backlash in the public, while democratic backsliding prompts the public to rally to democracy's cause.
This latter work takes advantage of recent advances in modeling public opinion as a latent variable to generate estimates of democratic support for over one hundred countries for up to nearly three decades, a much larger evidentiary base than any previous study.

The latent variable estimates of democratic support are based on source material consisting of thousands of nationally aggregated responses to dozens of different questions included in cross-national survey projects [@Claassen2020b, 40].
...
We re-collected all of the source data for the publication from the original surveys using `DCPOtools`, an R package which automates the process of aggregating survey responses for estimating dynamic comparative public opinion as a latent variable [@Solt2018].
...
We replicated each of the models presented in @Claassen2020b exactly, first with the article's original dataset and then making the corrections to the errors in the data we describe above.
...
The estimate is only slightly smaller than in the publication data, and as with the publication data, it disappears when corruption is added in Model 1.2: the evidence, such as it is, for the classic theory as operationalized here as a short-run process remains substantively unchanged.
...
Although failure to find support for a research hypothesis may prompt us to undertake a close review of the dataset to confirm that it is free of data-entry errors, an analysis that yields statistical significance is unlikely to trigger such a time-consuming and difficult effort.
This difference in the course taken depending on our data places us within 'the garden of forking paths,' rendering our findings suspect even when we only ever perform a single analysis [@Lohr2014, 464].

This leads to the second conclusion: to reduce the possibility of data-entry errors, researchers should minimize reliance on manual data entry and maximize the extent to which data collection and wrangling---the 'janitor work' of data analysis---is performed computationally.
...
Finally... [forking paths, Wuttke on credibility, extra-skeptical of feel-good results, thermostatic theory mechanism]
