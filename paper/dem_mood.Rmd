...
In this study, we leverage the @Claassen2020b to underscore the impact of data preprocessing on empirical outcomes, a largely underexplored but critical phase in research methodology. As @Weidmann2023 pointed out, although data collection, processing, and analyses can all influence the final outcomes, they follow distinct principles and require different attentions. 
Each of them is worth to specific attention, especially the data processing stage [@Weidmann2023, pp.4-9]
For those interested in the impacts of data analysis, we direct them to @TaiEtAl2022a, which provides an in-depth discussion on these issues in the context of a similar substantive topic.
]
...
Before elaborating on our discovery, it is noteworthy that it is impossible to tell the exact reason of the potential issues we identified just from the analytic outcomes.
The issues may be alternatively caused coding mistake or intentional decisions. 
Our focus here is, first, following @JanzFreese2021[p.306] to conduct a qualified scientific inquiry that "reflect[s] on typical robustness challenges" on some substantive conclusions and illustrate what suspicious operations probably caused by data janitor works can cause for an empirical research
...
We hope readers also consider the illustration and suggestions instead a replication of one single case, but an effort to echo the growing "open science" trend in and beyond social science to build more reliable and transparent research in general [@ChristensenEtAl2019; @EngzellRohrer2021].
...
These latent-variable estimates, in turn, were based on thousands of nationally aggregated responses to dozens of different questions from cross-national survey projects [@Claassen2020b, p.40].
...
To discern it requires close scrutiny of every manual entry; merely examining the data and their distribution will uncover few errors [@Barchard2011, pp. 1837-1838].
Although failure to find support for a research hypothesis may prompt us to undertake a such a close review, an analysis that yields statistical significance is unlikely to trigger what will likely be, as in the above example, a time-consuming and difficult effort [see @Gelman2014, p. 464].
...
Readers should be aware that our suggestion does *not* imply machine having universally superior reliability than human. 
Instead, the best utilization of machine coding should be always under delicate human designs and systemic supervisions [@Breznau2021; @Grimmer2015].
In the mean time, given the convenience of automatic data entry in documentation and reproduction, we encourage researchers to well utilize it instead of manually point-clicks to increase the efficacy and transparency of the data processing operations.^[See a systematic discussion of the function of automatic data process in social science in @Weidmann2023.]

Moreover, we are aware that, considering the practice cost, being open and transparent requires work [@EngzellRohrer2021].
But if researchers can automate some part of them, there is a higher chance that they can reuse the codes in the next time.
Many of them have already been packaged as open-source software that makes later researchers' task more straightforward and easier.^[See `readtext` [@Benoit2021] for formatting text files and `DCPOtools` [@Solt2018] for coding cross-sectional time-series public-opinion surveys.]
...
We hope the readers be aware that the above suggestions are closely related to our experience on a specific study. 
That means, although they successfully help us identify and fix data-entry concerns in this research, they are far from panacea to cure data processing problems in all types of studies. 
...
For instance, in the same vein of our first suggestion, @Weidmann2023 used a whole book to illustrate how to reduce "manual point and click" in a variety of studies with the `tidy`-data framework in R language.
@KapiszewskiKarcher2021[p.288] even suggested that researchers with qualitative method should consider using "open-exchange format" of qualitative data analysis software to improve "transparent about the generation and analysis of data."
Furthermore, we regard our efforts and recommendations as an additional effort to join the open science trend in social science [@ChristensenEtAl2019] and to produce data analytics that are robust and trusted (DART) more generally beyond social science [@BarchardPace2011; @Lohr2014].
With careful attention and specific efforts on data janitor work, not only can the threat of data-entry errors to our 'janitor work', our research, and our understanding of the world be minimized, but TOP^["Transparency and Openness Promotion", @NosekEtAl2015] knowledge can continuously grow.
